...
## Tightened Proof (AI Assisted Version)

*Note: I prefer to use my own writing, this is mainly included for clarity; it is, perhaps, more rigorous.*

*Definitions:*

**Ontology:** Let $(\mathcal{O}, \sqsubseteq)$ be an OWL ontology with an inferred $\sqsubseteq$ relation.

**Named Classes:** Let $N_C$ define the set of named classes in $\mathcal{O}$.

**Quasi-order:** The inferred $\sqsubseteq$ relation forms a quasi-order, satisfying the reflexive property $\forall C \in N_C : C \sqsubseteq C$ and the transitive property $C \sqsubseteq D \land D \sqsubseteq E$ implies $C \sqsubseteq E$. Note that a quasi-order does not satisfy antisymmetry.

**Equivalence:** Equivalence is defined $C \equiv D$ iff $C \sqsubseteq D \land D \sqsubseteq C$.

**Quotient Poset:** Given the $\sqsubseteq$ relation, as defined over $N_C$ in $\mathcal{O}$, the quotient $N_C/\equiv$ with order $[C] \leq [D] \iff C \sqsubseteq D$ forms a partial order, which is antisymmetric by construction, yielding a DAG.

**Transitive Reduction:** The transitive reduction over a partial order $(\mathcal{O}, \leq)$ removes edges $\langle p,q \rangle$ iff $\exists z \in \mathcal{O} : p < z < q$, i.e. removes edges with intermediate elements.

**Incomparable elements:** Two elements, $a,b$, are incomparable, denoted $a \parallel b$ iff $a \not\leq b \land b \not\leq a$.

**Pure hierarchy vs Poly-hierarchy:** For a pure hierarchy, each non-root node has exactly one parent in a hasse diagram; whereas, for a poly-hierarchy, at least one node has $>1$ (multiple) parents.

*Example*

Consider the following subsumption relationships for 'Viral pneumonia' in SNOMED CT:

- Let $X$ = 'Viral pneumonia (disorder)'
- Let $P$ = 'Pneumonia (disorder)'  
- Let $Q$ = 'Viral disease (disorder)'
- Let $Z$ = 'Disease (disorder)'

The following $\sqsubseteq$ relations are verified with Protégé:

1. $X \sqsubseteq P$ (Viral pneumonia is a pneumonia)
2. $X \sqsubseteq Q$ (Viral pneumonia is a viral disease)
3. $P \parallel Q$ (Pneumonia and Viral disease are incomparable)
...
Suppose an edge $\langle X,P \rangle$ is removed through transitive reduction. Then $\exists [W] : [X] < [W] < [P]$. Since $[X] \leq [Q]$ and we need to determine if $\langle X,Q \rangle$ is also transitive through $[W]$:

**Case 1:** 

- If $[W] \leq [Q]$, then since $[W] < [P]$, we have $[W] < [P]$ and $[W] \leq [Q]$.
- But $[P] \parallel [Q]$ implies $[P] \not\leq [Q]$.
- This would create $[W] \leq [Q]$ but $[P] \not\leq [Q]$ where $[W] < [P]$.
- By transitivity: if $[W] < [P]$ and $[W] \leq [Q]$, then $[P] \leq [Q]$. (Contradiction, since $[P] \parallel [Q]$).

**Case 2:**

- If $[Q] \leq [W]$, then since $[X] < [W]$, we have $[X] < [Q] \leq [W] < [P]$.
- This implies $[Q] < [P]$. (Contradiction, since $[Q] \parallel [P]$).

**Case 3:** 

- If $[W] \parallel [Q]$, then $\langle X,Q \rangle$ cannot be transitive through $[W]$.

$\therefore$ No single $[W]$ can make both edges transitive. At least one edge must remain. By symmetry, if $\langle X,Q \rangle$ is removed, $\langle X,P \rangle$ must remain. Since both cannot be removed, both are preserved. $\square$
